...
def craw(urlprm, cate):
    with oracle_engine.connect() as conn:
        ...
        for pageno in range(1, 7, 1): # 1~7페이지까지 크롤링
            try:
                interval = round(random.uniform(0.2, 1.2), 2) # 0.2~1.2초 sleep
                time.sleep(interval)
                #------------------------------------------------
                ...
                if response.status_code == 200:
                    ...
                    for i, li_tag in enumerate(list):
                        ...
                        dict['key_title'] = title  # 상세페이지 url
